...
# 中文系统提示
SYSTEM_PROMPT = (
    "你是一个中文人工智能助手。请根据要求，完成下面的数学竞赛题目。\n"
    "以下是中国数学竞赛中的解答题，答案类型为数值。请根据题目的要求和所提供的信息计算得出答案。\n"
    "请在最后以“所以最终答案是 \\boxed{答案}。”显式给出结果。\n"
    "过程解答过程和结果中使用的变量和公式请使用LaTeX格式表示。"
)
...
def get_olympiadbench_dataset(path: str = 'D:\\code\\py\\class_code\\nlp\\OlympiadBench_Dataset\\data\\OE_TO_maths_zh_COMP.json') -> Dataset:
...
training_args = GRPOConfig(
    ...
    output_dir="outputs/Qwen2.5-0.5B-OlympiadBench-GRPO",
)
...
# 训练
trainer.train()
trainer.save_model("outputs/Qwen2.5-0.5B-GRPO")
